...
const Highlights = (props: Record<string, any>) => {
    ...
    return !post ? <></> :
        <div key={post.title} className="w-full relative hidden lg:max-w-lg bg-white sm:block sm:p-8 lg:p-12 rounded-lg">
            <img src={post.media} className="hidden blog w-full ml-8 mb-4 md:block float-right cursor-pointer" style={{ maxWidth: "40%" }}  onClick={()=>onClickCard(post)}/>
            <div className="w-full">
                <h2 className="inline-flex items-center px-3 py-2 text-sm mb-6 font-medium bg-orange-100">
